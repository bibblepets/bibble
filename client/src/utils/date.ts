...
export const toAge = (birthDate: Date | string): string => {
  ...
  const diffWeeks = Math.round(Math.abs((today.getTime() - birthDate.getTime()) / oneWeek));
  const diffMonths = Math.round(Math.abs((today.getTime() - birthDate.getTime()) / oneMonth));
  const diffYears = Math.round(Math.abs((today.getTime() - birthDate.getTime()) / oneYear));
  ...
  if (diffDays === 0) {
    ...
    return `${weeks} ${weeks > 1 ? 'weeks' : 'week'}${days > 0 ? ` and ${days} ${days > 1 ? 'days' : 'day'}` : ''} old`;
  } else if (diffMonths < 12 && diffMonths > 0) {
    const months = Math.floor(diffDays / 30);
    const weeks = Math.floor((diffDays % 30) / 7);
    return `${months} ${months > 1 ? 'months' : 'month'}${weeks > 0 ? ` and ${weeks} ${weeks > 1 ? 'weeks' : 'week'}` : ''} old`;
  } else {
    const years = Math.floor(diffDays / 365);
    const months = Math.floor((diffDays % 365) / 30);
    return `${years} ${years > 1 ? 'years' : 'year'}${months > 0 ? ` and ${months} ${months > 1 ? 'months' : 'month'}` : ''} old`;
  }
};
...
export const toListingAge = (listedDate: Date | string): string => {
  ...
  const diffMinutes = Math.round(Math.abs((today.getTime() - listedDate.getTime()) / oneMinute));
  const diffHours = Math.floor((today.getTime() - listedDate.getTime()) / (1000 * 60 * 60));
